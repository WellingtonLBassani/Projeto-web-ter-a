    ...
    @foreach($pokemon as $entity)
        <div class="p-4 max-w-sm">
            <div class="flex rounded-lg h-full bg-teal-400 p-8 flex-col">
                <img class="w-24 h-24 mb-3 rounded-full shadow-lg" src="{{ asset($entity->image) }}" alt="{{ $entity->nome }}"/>
                <h5 class="mb-1 text-xl font-medium text-gray-900">{{ $entity->nome }}</h5>
                <span class="text-sm text-gray-500">{{ $entity->tipo }}</span>
                <span class="text-sm text-gray-500">{{ $entity->pontosdepoder }}</span>
                @if(isset($entity->coach))
                    <span class="text-sm text-gray-500">{{ $entity->coach->nome }}</span>
                @else
